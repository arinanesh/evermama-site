...
- `sitemap.xml` lists every canonical URL (home, blog index, each post, support, privacy, delete-account) with a `lastmod` per page. **When you add a page or a blog post, add it to the sitemap and bump `lastmod` when you change a page's content.** `robots.txt` points at it (Cloudflare prepends its managed content-signals block; that's expected).
...
## SEO conventions (September 2026)

- **`.assetsignore` is the deploy allowlist's other half.** Cloudflare Workers static assets upload *every* file under `.` (the assets directory), including `.git/`, `.wrangler/`, `CLAUDE.md`, `README.md`, `wrangler.jsonc`, `blog/_template.html`, `tmp/` and the raw device captures. Before September 2026 all of those were live at `evermama.app` (including `/.git/config`). Wrangler ignores nothing by default; `.assetsignore` (gitignore syntax) lists what stays private. When you add a scratch file, draft, or tooling artifact to the repo, add it there too, or it ships.
- **Every page carries the same social block**: `og:site_name`, `og:type`, `og:url` (= canonical), `og:title`, `og:description`, an *absolute* `og:image` with width/height, and the matching `twitter:*` set. The home page and posts use `summary_large_image` with the hero illustration (1080 × 520 / 1000 × 667); doc pages use `summary` with `assets/icon.png`.
- **Structured data (JSON-LD)**: `index.html` declares `Organization`, `WebSite` and a free `MobileApplication` (`@graph`); `support.html` declares `FAQPage` mirroring the visible questions (keep them in sync when editing the FAQ); `blog/index.html` declares `Blog` with a `blogPost` list (add each new post); every post declares `BlogPosting` with `datePublished` / `dateModified`, the hero image, and EverMama as author and publisher. The blog template carries the slots. Author is the Organization, not a named person, on purpose: the site never names the founder.
- **Titles**: `<Page> — EverMama` with an em dash, except when the title already contains the brand. The home title leads with the plain phrase "healthy habits for new moms" (the search-trends report's echo phrase), and the meta description pairs the decades-long promise with the this-week benefit.
- **Illustrations ship as PNG + WebP siblings** in `<picture>` (WebP first, PNG fallback), generated with `sharp(...).webp({quality: 90})` from the sibling repo's `node_modules`. WebP q90 halves each file with no visible change to the paintings. The PNG stays the `og:image` and the lightbox source. Screenshots stay PNG-only. Above-the-fold heroes carry `fetchpriority="high"`; below-the-fold images carry `loading="lazy"`.
- **Posts cross-link**: each post's `.post-foot` links two other posts ("More from the blog"), and the blog index must list every post, or the post is an orphan for crawlers.
- **`npx wrangler dev` reload loop**: with `assets.directory` set to `.`, wrangler watches its own `.wrangler/tmp` output and reloads forever, and pages never load. Use `npx wrangler deploy --dry-run` to check the asset set, or preview with `python -m http.server` and accept that extensionless links won't resolve.
